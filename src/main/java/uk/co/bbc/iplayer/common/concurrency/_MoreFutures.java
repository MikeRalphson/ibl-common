 ...
 */
@Deprecated
public class _MoreFutures {

    private final static boolean INTERRUPT_TASK = true;
    private static final Logger LOG = LoggerFactory.getLogger(_MoreFutures.class);

    private _MoreFutures() {
    ...
    private static <T> List<T> filterCompleteTasks(Iterable<? extends ListenableFuture<? extends T>> futures) {
        ...
        for (ListenableFuture<? extends T> future : futures) {
            ...
            if (future.isDone() && !future.isCancelled()) {
                T value = null;
                try {
                    value = _MoreFutures.await(future, Duration.inMilliSeconds(10));
